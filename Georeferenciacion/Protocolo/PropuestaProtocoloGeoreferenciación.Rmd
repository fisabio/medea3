...
La librería *caRtociudad* puede instalarse desde la web de [*GitHub*](https://github.com/cjgb/caRtociudad) mediante el siguiente código de *R*:
...
ejemplo2$DIREC <- with(ejemplo2, paste(TVIA, NVIA, NUMEROPOLI, ", ",
...
Consultando el objeto `resultados_cartociudad$state` podemos valorar el éxito de la geocodificación llevada a cabo. En nuestro ejemplo, con 30 registros, 14 de ellos han obtenido *state* igual `1`, uno obtuvo *state* de `2`, tres obtuvieron state igual a `3`, y diez tienen state igual a `10`. Daremos por geocodificados los 20 primeros mientras que los 10 últimos intentaremos su geocodificación con la herramienta de *Google*.
