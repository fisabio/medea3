---
title: "Propuesta de protocolo de geocodificación (Versión 1.0)"
...
donde *url* es la dirección IP o dirección web del proxy de vuestra red, *port* es el puerto que vuestra institución utilice para comunicaciones y *username* y *password* las credenciales de acceso, si fueran necesarias. Si no necesitais identificar usuario y contraseña en vuestra red quitar dichos arcumentos de la sentencia anterior. Los valores de nuestra red los hemos enmascarado por seguridad. Las comillas, allá donde aparecen son necesarias. Vuestros servicios de informática (suponemos que) podrán proporcionaros información con más detalle sobre estos valores para vuestras redes informáticas corporativas.
...
![](figuras/salida_caRtociudad.png)


El resultado contiene información que identifica el lugar o lugares geocodificados, en el caso de que haya más de una propuesta para una misma dirección. En el campo **`priority`** se devuelve el valor **`1`** cuando hay un único resultado, y se establece el orden del más probable al menos probable cuando hay varias propuestas. Por otro lado, el campo **`status`** contiene información sobre la confianza/precisión de las coordenadas proporcionadas. Esta variable **`status`** puede tomar como valor:

- **1** : *Portal/Pk encontrado*
- **2** : *Portal/Pk no encontrado*. Se devuelve el más cercano encontrado. En el campo `numpk_name`  nos reporta qué portal nos ha dado.
- **3** : *Portal/Pk no encontrado*. Se devuelve el inicial de la vía
- **4** : *Municipio encontrado*, pero no la vía
- **0** : Recoge diferentes situaciones. Por un lado vías encontradas que no tienen números asociados (entendemos que en su sistema), y por otro viales no encontrados. Devuelve el comentario *El vial encontrado (NOMBRE DEL VIAL) no tiene Portal/Pk asociados* o el comentario *Vial no encontrado*. En el primer caso devuelve una geocodificación (que corresponde a un punto central del municipio habitualmente), en el segundo no devuelve ninguna geocodificación. Los códigos de status mñas habituales, de largo, son los `1`, `2` y `0`.

En los campos **`longitude`** y **`latitude`** disponemos de la longitud y la latitud devuelta por *CartoCiudad*. Esta información (longitud, latitud y el resto de campos mencionados) puede ser almacenada en un banco de datos, junto con el resto de información disponible en la base de datos de mortalidad.

## Normalización direcciones para *CartoCiudad*.

En el documento que se encuentra en [http://www.idee.es/resources/presentaciones/JIIDE10/ID449_Servicio_web_de_Geocodificacion_para_Cartociudad.pdf](http://www.idee.es/resources/presentaciones/JIIDE10/ID449_Servicio_web_de_Geocodificacion_para_Cartociudad.pdf)
se comentan algunos detalles interesantes sobre el servicio web de geocodificación con *CartoCiudad*. Entre otros detalles se describe qué formato se recomienda para las direcciones que queramos geocodificar. Existe la posibilidad de enviar a geocodificar una dirección separando cada elemento de la misma en una variable distinta que pasaremos a la función **`geocode`**, o hacerlo de forma libre mediante una única cadena de texto. En nuestra experiencia la segunda forma, la de texto libre, nos ha reportado alguna mejora en los resultados. En ocasiones, la información en alguno de nuestros registros a geocodificar no está exactamente en el campo que debiera pudiendo tener por ejemplo el tipo de vía vacío y en el nombre de vía tener tanto el tipo como el nombre. Por este motivo nuestra recomendación es optar por esta segunda forma, el texto libre.
...
5. Extracción del **número de policía**. Se busca este valor al **final** de la cadena de texto. Si se encuentra un valor numérico en dicha posición se extrae y elimina de la cadena de texto.
...
En la página web [https://developers.google.com/maps/documentation/geocoding/intro#geocoding](https://developers.google.com/maps/documentation/geocoding/intro#geocoding) se puede ampliar la información aquí resumida que hemos considerado de particular interés para el proyecto.
...
ejemplo2$DIREC<-paste(ejemplo2$TVIA," ",ejemplo2$NVIA,", ",ejemplo2$NUMEROPOLI,", ", ejemplo2$MUNICIPIO," , ",ejemplo2$PROVINCIA, ",", ejemplo2$CP, sep="")

```

## *caRtociudad*
A continuación describimos el proceso en el que, para nuestro ejemplo, generamos una estructura de datos anexa al data.frame *`ejemplo2`* original que nos permitirá guardar la información que nos devuelva *caRtociudad* y lanzamos la geocodificación.
```{r}
library(caRtociudad)

#Creamos la estructura para recoger la información que proporciona CartoCiudad
direcgeo<-data.frame(matrix(nrow=dim(ejemplo2)[1],ncol=14))
#Para diferenciar que esta información es la proporcionada por CartoCiudad añadimos al nombre de las columnas que devolverá CartoCiudad la pablabra "carto"
colnames(direcgeo)<-paste("carto",c("priority","status","comments","province","municipality",
                                    "road_type","road_name","road_fid","numpk_name","zip","numpk_fid",
                                    "longitude","latitude","numdirec"),sep=".")

#Anexionamos esta estructura a nuestro banco de datos original
ejemplo2<-data.frame(ejemplo2,direcgeo)
```

A continuación, las siguientes sentencias mandan a geocodificar cada una de las direcciones de *`ejemplo2`* y guardan la información original de cada dirección en el propio data.frame *`ejemplo2`*
```{r}
#Aquí mostramos cómo hacerlo dirección a dirección, aunque se podría enviar bloques de direcciones
for (i in 1:dim(ejemplo2)[1]){
  direc<-ejemplo2$DIREC[i]
  aux<-cartociudad_geocode(direc)
  ejemplo2[i,colnames(direcgeo)]<-aux[1,]
  cat(paste(i,aux$status[1],"\n"))
}

```

Notar que el bucle anterior se ejecuta de forma individual para cada una de las direcciones disponibles cuando *CartoCiudad* admite la posibilidad de hacer esta labor para bloques de hasta 100 direcciones. En general preferimos ejecutar el bucle dirección a dirección ya que en caso de que el proceso diera algún error en algún momento (ya sea por fallo de la red o de *CartoCiudad*) y parara el bucle, habríamos guardado toda la información hasta ese punto. En caso de geocodificar en bloque un conjunto de direcciones si el proceso fallara no devolvería ninguna información para ninguna de las direcciones del bloque.

Consultando el objeto `ejemplo2$carto.status` podemos valorar el éxito de la geocodificación llevada a cabo. En nuestro ejemplo, con 30 registros, 13 de ellos han obtenido status `1`, 7 registros han obtenido status `2` y 10 tienen status `0`. Daremos por geocodificados los 20 primeros mientras que los 10 últimos intentaremos su geocodificación con la herramienta de *Google*.
...
#Creamos la estructura para recoger la información que proporciona CartoCiudad
resulgoogle<-data.frame(matrix(nrow=dim(ejemplo2)[1],ncol=5))
colnames(resulgoogle)<-c("google.STATUS","google.latitude","google.longitude","google.type","google.formatted_address")

#Anexionamos esta estructura a nuestro banco de datos original
ejemplo2<-data.frame(ejemplo2,resulgoogle)
```
...
selec<-which(ejemplo2$carto.status %in% c("0","4"))
 for (i in selec){
    direc<-ejemplo2$DIREC[i]
    #La opción override_limit parece que permite realizar algunas geocodificaciones extra por encima del límite diario, a costa de que pueda haber alguna penalización extra en el límite diario de los días posteriores.
    aux<-geocode(as.character(direc),output="all", source="google", override_limit = TRUE)
    if (is.na(aux)==FALSE){
      ejemplo2$google.STATUS[i]<-aux$status
      if (length(aux$results)>0){
        ejemplo2$google.latitude[i]<-aux$results[[1]]$geometry$location$lat
        ejemplo2$google.longitude[i]<-aux$results[[1]]$geometry$location$lng
        if (length(aux$results[[1]]$types) > 0){
          ejemplo2$google.type[i]<-aux$results[[1]]$types[[1]]
        }
        ejemplo2$google.formatted_address[i]<-aux$results[[1]]$formatted_address
      }
    }
 }

```
Notar que en las sentencias anteriores el bucle nuevamente geocodifica dirección a dirección aunque se podría hacer por tandas de grupos de direcciones. En cualquier caso, por los mismos argumentos que para *CartoCiudad*, se ha considerado oportuno hacerlo dirección a dirección. También resulta interesante comentar que aunque *Google* devuelve un montón de información sobre la dirección que se ha geocodificado sólo nos guardamos aquella que nos interesa por cuestiones operativas. 
...
```{r}
  selec<-which((ejemplo2$google.STATUS %in% c("OVER_QUERY_LIMIT","UNKNOWN_ERROR")) | is.na(ejemplo2$google.STATUS)==TRUE)
  for (i in selec){
    #mismo procedimiento que en el for del bloque de código anterior
  }

```
Según el procedimiento expuesto, los valores de latitud y longitud de los registros geocodificados se encuentran en las variables `carto.latitude` y `carto.longitude` para los registros cuyo valor en `carto.status` es `1`, `2` y `3`, y en los campos `google.latitude` y `google.longitude` para los campos en los que `carto.status` es `0` o `4` y `google.status` es `OK`. El resto de registros serían los que no se han podido geocodificar y que deben desestimarse o pasar a un proceso de revisión manual en función de la proporción del total que supongan estos registros y de los recursos del grupo que lleva a cabo el proceso.
...
```{r}
#Los que no hayan podido ser geocodificados se quedarán con coordenadas (0,0), ya que los valores perdidos NA hacen que algunos procesos posteriores no funcionen bien.
ejemplo2$latitude<-rep(0,dim(ejemplo2)[1])
ejemplo2$longitude<-rep(0,dim(ejemplo2)[1])

#Los geocodificados con caRtociudad
seleccarto<-which(ejemplo2$carto.status %in% c("1","2","3"))
ejemplo2$latitude[seleccarto]<-ejemplo2$carto.latitude[seleccarto]
ejemplo2$longitude[seleccarto]<-ejemplo2$carto.longitude[seleccarto]

#Los geocodificados con *Google*

selecgoogle<-which((ejemplo2$carto.status %in% c("0","4")) & ejemplo2$google.STATUS=="OK")
ejemplo2$latitude[selecgoogle]<-ejemplo2$google.latitude[seleccarto]
ejemplo2$longitude[selecgoogle]<-ejemplo2$google.longitude[seleccarto]
...
#Leemos cartografía (entre los archivos de la cartografía debe haber uno con extensión .prj). Sustituir "ruta cartografia" y "nombrefichero" por los argumentos oportunos.
carto<-readOGR("...ruta cartografia.../nombrefichero.shp",layer="nombrefichero")

#Definimos un objeto con las coordenadas de las defunciones y calculamos su proyección según los parámetros utilizados por CartoCiudad y Google para asignarles las coordenadas.
...
#Transformamos la proyección de los puntos al mismo sistema de coordenadas de la cartografía para que las coordenadas de ambas fuentes de información coincidan.
puntos.trans<-spTransform(puntos,CRS(proj4string(carto)))

#Por último, asignamos la sección censal que corresponde a cada punto con la función over. En nuestra cartografía el campo DISTRSEC contiene el Identificador de cada sección censal que vamos a asignar a las defunciones. Si en vuestras cartografías dicho identificador se llamara de forma distinta habríais de sustituir DISTRSEC en la siguiente expresión por dicho identificador. 
ejemplo2$SC<-over(puntos.trans,carto)$DISTRSEC
...
# Anexo: Funciones de limpieza y normalización de datos.
